...
template<
  ...
  bool OPTIMIZED_FOR_LARGE_GROUPS = false>
__device__ void hybrid_paged_attention_kernel(
  ...
  const int divergence_block_idx,
  const int* __restrict__ sequence_block_offsets
) {
  ...
  // Calculate the number of sequences per warp.
  const int num_seqs_per_warp = NUM_THREADS / WARP_SIZE;

  // Shared prefix processing (optimized kernel).
  if (divergence_block_idx > start_block_idx) {
    // Iterate over shared KV cache blocks.
    for (int block_idx = start_block_idx + warp_idx; block_idx < divergence_block_idx; block_idx += NUM_WARPS) {
...
#pragma unroll
        ...
        // Iterate over sequences within the warp.
        for (int seq_idx_in_warp = 0; seq_idx_in_warp < num_seqs_per_warp; ++seq_idx_in_warp) {
          const int seq_idx = block_idx * num_seqs_per_warp + seq_idx_in_warp;

          // Check if the sequence is valid.
          const int context_len = context_lens[seq_idx];
          if (USE_PARTITIONING && partition_idx * PARTITION_SIZE >= context_len) {
            // No work to do for this sequence.
            continue;
          }
...
#pragma unroll
  ...
  // Divergent suffix processing (normal kernel).
  for (int block_idx = max(divergence_block_idx, start_block_idx) + warp_idx; block_idx < end_block_idx; block_idx += NUM_WARPS) {
    // Get the current block index for the sequence.
    const int seq_block_idx = sequence_block_offsets[threadIdx.x];

    // Check if the block index is valid for the sequence.
    if (seq_block_idx < num_context_blocks) {
      // NOTE(woosuk): The block number is stored in int32. However, we cast it to int64
      // because int32 can lead to overflow when this variable is multiplied by large numbers
      // (e.g., kv_block_stride).
      const int64_t physical_block_number = static_cast<int64_t>(block_table[seq_block_idx]);
...
#pragma unroll
    ...
    sequence_block_offsets[threadIdx.x]++;
  }
...
__global__ void paged_attention_v3_kernel(
  ...
  hybrid_paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_E5M2_KV_CACHE>(
    /* exp_sums */ nullptr, /* max_logits */ nullptr,
    out, q, k_cache, v_cache, num_kv_heads, scale, block_tables, context_lens,
    max_num_blocks_per_seq, alibi_slopes, q_stride, kv_block_stride, kv_head_stride,
    divergence_block_idx, sequence_block_offsets
  );
}
...
#define LAUNCH_PAGED_ATTENTION_V3(HEAD_SIZE)                                                  \
  VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                                       \
    ((void*)vllm::paged_attention_v3_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,   \
      IS_FP8_E5M2_KV_CACHE>), shared_mem_size);                                               \
  vllm::paged_attention_v3_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,             \
  IS_FP8_E5M2_KV_CACHE><<<grid, block, shared_mem_size, stream>>>(                            \
    out_ptr,                                                                                  \
    query_ptr,                                                                                \
    key_cache_ptr,                                                                            \
...
void paged_attention_v3_launcher(
  ...
  int max_context_len,
  const c10::optional<torch::Tensor>& alibi_slopes,
  int divergence_block_idx,
  torch::Tensor& sequence_block_offsets
) {
  int num_seqs = query.size(0);
  int num_heads = query.size(1);
  int head_size = query.size(2);
  ...
  int* block_tables_ptr = block_tables.data_ptr<int>();
  int* context_lens_ptr = context_lens.data_ptr<int>();
  int* sequence_block_offsets_ptr = sequence_block_offsets.data_ptr<int>();
...
#define CALL_V3_LAUNCHER(T, CACHE_T, BLOCK_SIZE, IS_FP8_E5M2_KV_CACHE)       \
  paged_attention_v3_launcher<T, CACHE_T, BLOCK_SIZE, IS_FP8_E5M2_KV_CACHE>( \
    out,                                                                     \
    query,                                                                   \
    key_cache,                                                               \
    value_cache,                                                             \
...
// NOTE(woosuk): To reduce the compilation time, we omitted block sizes
// 1, 2, 4, 64, 128, 256.
#define CALL_V3_LAUNCHER_BLOCK_SIZE(T, CACHE_T, IS_FP8_E5M2_KV_CACHE) \
  switch (block_size) {                                               \
    case 8:                                                           \
      CALL_V3_LAUNCHER(T, CACHE_T, 8, IS_FP8_E5M2_KV_CACHE);          \
      break;                                                          \
    case 16:                                                          \
      CALL_V3_LAUNCHER(T, CACHE_T, 16, IS_FP8_E5M2_KV_CACHE);         \
      break;                                                          \
    case 32:                                                          \
      CALL_V3_LAUNCHER(T, CACHE_T, 32, IS_FP8_E5M2_KV_CACHE);         \
      break;                                                          \
    default:                                                          \
      TORCH_CHECK(false, "Unsupported block size: ", block_size);     \
      break;                                                          \
...
void paged_attention_v3(
  ...
  if (kv_cache_dtype == "auto") {
    if (query.dtype() == at::ScalarType::Float) {
      CALL_V3_LAUNCHER_BLOCK_SIZE(float, float, false);
    } else if (query.dtype() == at::ScalarType::Half) {
      CALL_V3_LAUNCHER_BLOCK_SIZE(uint16_t, uint16_t, false);
    } else if (query.dtype() == at::ScalarType::BFloat16) {
      CALL_V3_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, __nv_bfloat16, false);
    } else {
      TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
    }
  } else if (kv_cache_dtype == "fp8_e5m2") {
    if (query.dtype() == at::ScalarType::Float) {
      CALL_V3_LAUNCHER_BLOCK_SIZE(float, uint8_t, true);
    } else if (query.dtype() == at::ScalarType::Half) {
      CALL_V3_LAUNCHER_BLOCK_SIZE(uint16_t, uint8_t, true);
    } else if (query.dtype() == at::ScalarType::BFloat16) {
      CALL_V3_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, uint8_t, true);
    } else {
      TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
    }
  } else {
